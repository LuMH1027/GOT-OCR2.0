...
def eval_model(args):
    ...
    model = GOTQwenForCausalLM.from_pretrained(
        model_name, low_cpu_mem_usage=True, device_map='cuda', use_safetensors=True, pad_token_id=151643).eval()
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        if args.render:
            ...
            if '**kern' in outputs:
                ...
                tk.getPageCount()
                svg = tk.renderToSVG()
                svg = svg.replace("overflow=\"inherit\"",
                                  "overflow=\"visible\"")
            ...
            if args.type == 'format' and '**kern' not in outputs:
                ...
                if '\\begin{tikzpicture}' not in outputs:
                    ...
                    if right_num != left_num:
                        outputs = outputs.replace('\left(', '(').replace('\\right)', ')').replace('\left[', '[').replace('\\right]', ']').replace(
                            '\left{', '{').replace('\\right}', '}').replace('\left|', '|').replace('\\right|', '|').replace('\left.', '.').replace('\\right.', '.')
