...
class GOTQwenModel(Qwen2Model):
    ...
    def initialize_vision_modules(
        ...
        self.vision_tower_high = self.vision_tower_high.to(
            dtype=dtype, device=device)

        self.mm_projector_vary = self.mm_projector_vary.to(
            dtype=dtype, device=device)
    ...
    def forward(
        ...
        if orig_embeds_params is not None:
            with torch.no_grad():
                ...
                ).weight[:-self.num_new_tokens] = orig_embeds_params[:-self.num_new_tokens].data
        ...
        if vision_tower_high is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            freeze_vision_tower = getattr(
                self.config, "freeze_vision_tower", False)
            ...
            #     print(f"type(image[1]): {type(image[1])}")
            for image in images:
                # print(f"image[1].shape: {image[1].shape}")
                # print(f"type(image[1]): {type(image[1])}")
                ...
                if P == 1:
                    ...
                    for image_patch in image_patches:
                        ...
                        with torch.set_grad_enabled(False):
                            ...
                            cnn_feature_p = cnn_feature_p.flatten(
                                2).permute(0, 2, 1)
...
class GOTQwenForCausalLM(Qwen2ForCausalLM):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # print(
            #     f"shift_logits: {shift_logits.shape}, type: {shift_logits.dtype}")
            # print(
            #     f"shift_labels: {shift_labels.shape}, type: {shift_labels.dtype}")
