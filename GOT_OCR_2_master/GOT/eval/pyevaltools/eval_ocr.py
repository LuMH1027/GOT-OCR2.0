...
def nougat_per_metrics(predict_root_, pred, gt, minlen=1, heavy_mode: int = 2):
    ...
    if heavy_mode == 2:
        # 速度太慢
        metrics["edit_dist"] = nltk.edit_distance(
            pred, gt) / max(len(pred), len(gt))
    return metrics
...
def doc_formated_text_eval(gt_root_, predict_root_, datatype):
    ...
    gt_text_split, gt_math_split, gt_table_split = split_text(
        predicts, 'label')
    pre_text_split, pre_math_split, pre_table_split = split_text(
        predicts, 'answer')
    text_results = []
    math_results = []
    table_results = []
...
def doc_text_eval(gt_root_, predict_root_, datatype):
    ...
    for ann in predicts:
        try:
            ans = nougat_per_metrics(
                predict_root_, ann["label"], ann["answer"])
            if len(ans) == 0:
                continue
            result.append(ans)
        except:
...
doc_text_eval(args.gt_path, args.out_path +
              "/results_final.json", args.datatype)
