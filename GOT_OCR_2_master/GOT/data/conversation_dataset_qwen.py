...
class ConversationDataset(BaseDataset):
    ...
    def __init__(self, datasets, tokenizer, multimodal_cfg, augment: bool = False,):
        ...
        got_data_dict = {
            ...
            "data_3": ["data_3"],
            "data_4": ["data_4"],
            "test-longest-gpt-replies": ["data_5"],
            # "pdf-ocr": ["data1", "data2"],
        ...
        for name_all in datasets.split("+"):
            for name in got_data_dict[name_all]:
                ...
                for d in json.load(open(data_path, "r")):
                    if "tokenized" in d:
                        input_ids = d["tokenized"].get("input_ids", [])
                        if isinstance(input_ids, list) and len(input_ids) <= 600:
                            filtered_data.append(d)
                            filtered_image_path.append(dataset['images'])
                        else:
                            logging.warning(
                                f"Data from {data_path} has invalid input_ids length: {len(input_ids)}. Skipping this entry.")
                    else:
                        # 保留未预先tokenize的数据（可选）
                        filtered_data.append(d)
                        filtered_image_path.append(dataset['images'])

                list_data_dict.extend(filtered_data)
                list_image_path.extend(filtered_image_path)

                logging.warning(
                    f"Data from {data_path} provide {len(filtered_data)} valid conversations.")
                ...
                # logging.warning(
                #     f"Data from {data_path} provide {len(data)} conversations.")
    ...
    def multimodal_processor(self, sources, flag_num_patches):
        for source in sources:
            if self.multimodal_cfg['sep_image_conv_front']:
                ...
                source[0]['value'] = source[0]['value'].replace(
        ...
        for conversation, target in zip(conversations, targets):
            ...
            for i, rou in enumerate(re_rounds):
                ...
                round_len = len(self.tokenizer(rou).input_ids) + \
                    len(self.tokenizer(conv.sep).input_ids)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "tokenized" in data:
            input_ids = torch.tensor(data["tokenized"]["input_ids"])
            labels = torch.tensor(data["tokenized"]["labels"])
            data_dict = dict(input_ids=input_ids, labels=labels)
            # data_dict_ori = self.token_processor(conversations, image_name)
            # data_dict_ori = dict(
            #     input_ids=data_dict_ori["input_ids"][0], labels=data_dict_ori["labels"][0])
            # print("data_dict_ori", data_dict_ori["input_ids"].shape)
            # print("data_dict", data_dict["input_ids"].shape)
        else:
            data_dict = self.token_processor(conversations, image_name)
            data_dict = dict(
                input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0])
            # print("data_dict", data_dict["input_ids"])
